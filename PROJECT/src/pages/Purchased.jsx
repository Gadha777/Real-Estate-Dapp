  ...
  useEffect(() => {
    const fetchPurchasedPlots = async () => {
      try {
        ...
        // Call the `viewBoughtPlots` function from the contract
        const BoughtPlots = await realEstateInstance.viewBoughtPlots();

        // Parse the results and update the state
        const plots = BoughtPlots.map((plot) => ({
          id: plot.id.toString(),
          name: plot.name,
